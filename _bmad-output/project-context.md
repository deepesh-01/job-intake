...
- **Apply Co-Pilot** lives under `src/apply/`. Headful Playwright Chromium-for-Testing, persistent-context per host under `data/playwright/<host>/` (cookies cached). Profile YAML at `data/apply_profile.yaml` (gitignored — phone, LinkedIn URL, CTC). Per-run logs at `data/apply_runs/<safe_id>_<ts>.log`. Submit-block injected via `page.evaluate` with strict immediate-target check (no DOM-parent walk, or Next/Continue buttons get false-positive blocked). See ADR-027.
